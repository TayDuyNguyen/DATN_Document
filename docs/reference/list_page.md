...
| 4.2 | Chi tiết Đơn hàng | `/admin/bookings/{id}` | Detail + actions | `GET /admin/bookings/{id}` `GET /admin/bookings/{id}/passengers` *(planned)* `GET /admin/bookings/{id}/timeline` *(planned)* `PATCH /admin/bookings/{id}/status` *(confirm/cancel/complete qua `booking_status`; API action riêng `/confirm`, `/cancel`, `/complete` chưa có route)* |
...
| # | Màn hình | Route | Loại | API sử dụng |
|---|----------|-------|------|-------------|
| 9.1 | Danh sách Khuyến mãi | `/admin/promotions` | List + filter + paginate | *(planned)* `GET /admin/promotions` `GET /admin/promotions/{id}` `PATCH /admin/promotions/{id}/status` `DELETE /admin/promotions/{id}` |
| 9.2 | Tạo Khuyến mãi | `/admin/promotions/create` | Form | *(planned)* `POST /admin/promotions` |
| 9.3 | Chỉnh sửa Khuyến mãi | `/admin/promotions/{id}/edit` | Form | *(planned)* `GET /admin/promotions/{id}` `PUT /admin/promotions/{id}` |
...
| # | Màn hình | Route | Loại | API sử dụng |
|---|----------|-------|------|-------------|
| 10.1 | Danh sách Thông báo | `/admin/notifications` | List + filter | `GET /admin/notifications` `DELETE /admin/notifications/{id}` |
| 10.2 | Gửi Thông báo | `/admin/notifications/send` | Form | `POST /admin/notifications/send` `POST /admin/notifications/send-all` `GET /admin/users` *(chọn user)* |
...
| # | Màn hình | Route | Loại | API sử dụng |
|---|----------|-------|------|-------------|
| 11.1 | Danh sách Liên hệ | `/admin/contacts` | List + filter + paginate | `GET /admin/contacts` `DELETE /admin/contacts/{id}` `GET /admin/contacts/export` |
| 11.2 | Chi tiết & Trả lời | `/admin/contacts/{id}` | Detail + form reply | `GET /admin/contacts/{id}` `POST /admin/contacts/{id}/reply` `DELETE /admin/contacts/{id}` |
...
| # | Màn hình | Route | Loại | API sử dụng |
|---|----------|-------|------|-------------|
| 12.1 | Cấu hình Website | `/admin/settings` | Form nhiều nhóm | *(planned)* `GET /admin/settings` `PUT /admin/settings` |
| 12.2 | Landing Pages | `/admin/landing-pages` | List + CRUD | *(planned)* `GET /admin/landing-pages` `POST /admin/landing-pages` `PUT /admin/landing-pages/{id}` `PATCH /admin/landing-pages/{id}/status` |
...
| # | Màn hình | Route | Loại | API sử dụng |
|---|----------|-------|------|-------------|
| 13.1 | Báo cáo Đơn hàng | `/admin/reports/bookings` | Chart + table + export | `GET /admin/reports/bookings` `GET /admin/bookings/export` |
| 13.2 | Báo cáo Doanh thu | `/admin/reports/revenue` | Chart + table + export | `GET /admin/reports/revenue-detail` `GET /admin/dashboard/revenue` `GET /admin/payments/export` |
| 13.3 | Báo cáo Đánh giá | `/admin/reports/ratings` | Chart + table + export | `GET /admin/reports/ratings` `GET /admin/ratings/export` |
| 13.4 | Báo cáo Địa điểm | `/admin/reports/locations` | Chart + table + export | `GET /admin/reports/locations` `GET /admin/locations/export` |
| 13.5 | Báo cáo Người dùng | `/admin/reports/users` | Chart + export | `GET /admin/reports/users` `GET /admin/users/export` |
...
| 9 | Khuyến mãi | 3 | Planned: list · create · edit |
| 10 | Thông báo | 2 | List · Gửi thông báo |
| 11 | Liên hệ | 2 | List · Detail/Reply |
| 12 | Cấu hình | 2 | Planned: website settings · landing pages |
| 13 | Báo cáo | 5 | Đơn hàng · Doanh thu · Đánh giá · Địa điểm · Người dùng |
| | **Tổng** | **48** | 43 hiện có/mô tả + 5 planned |
...
- Các màn planned đã có file mô tả ở mức nghiệp vụ: `admin_promotions.md`, `admin_site_settings.md`, `admin_landing_pages.md`.
- Các màn báo cáo/chi tiết bổ sung đã có file mô tả: `admin_payments_detail.md`, `admin_reports_locations.md`, `admin_reports_users.md`.
- Thao tác đơn hàng hiện dùng route thật `PATCH /admin/bookings/{id}/status`; các route action riêng `POST /admin/bookings/{id}/confirm`, `POST /admin/bookings/{id}/cancel`, `POST /admin/bookings/{id}/complete` chỉ là API planned nếu muốn tách action rõ hơn.
...
|---|---|---|---|---|
| Cao | Quản lý khuyến mãi | `/admin/promotions` | Đã đưa vào nhóm 9 | `admin_promotions.md` |
| Cao | Cấu hình website | `/admin/settings` | Đã đưa vào nhóm 12 | `admin_site_settings.md` |
| Cao | Quản lý tour nâng cao | `/admin/tours/create`, `/admin/tours/{id}/edit` | Cập nhật màn hiện có | `admin_tours_create.md`, `admin_tours_edit.md`, `admin_tours_detail.md` |
| Cao | Quản lý lịch khởi hành nâng cao | `/admin/tour-schedules` | Cập nhật màn hiện có | `admin_tour_schedules_list.md`, `admin_tour_schedules_create.md`, `admin_tour_schedules_edit.md` |
