...
- Button Yêu thích:
  ...
  - Đã thích: icon `favorite #EF4444` → `DELETE /user/favorites` body/query `location_id`
...
| Kiểm tra đã yêu thích (🔐) | GET | `/user/favorites/check?location_id={location_id}` | Khi mount |

---

## Điểm khác biệt khi đã đăng nhập (🔐)

### Button Yêu thích — Active state

**API khi mount: `GET /user/favorites/check?location_id={location_id}`**

| Trạng thái | Icon | Style | Action |
|-----------|------|-------|--------|
| Chưa yêu thích | `favorite_border` | `border #E2E8F0 bg white color #94A3B8` | `POST /user/favorites` |
| Đã yêu thích | `favorite` | `border #EF4444 bg #FEE2E2 color #EF4444` | `DELETE /user/favorites` body `location_id` |
...
Button xuất hiện ở **2 vị trí**: Header bên phải (Section 3.1) và Sidebar Card (Section 4.3)

### Button Viết đánh giá — Unlock
...
**Stars selector:** `flex gap-8 justify-center my-16`
- 5 stars `36px` clickable, hover fill từ trái → phải `#F59E0B`
- Label: `"Tệ" / "Không tốt" / "Bình thường" / "Tốt" / "Tuyệt vời"` `13px #94A3B8`

**Textarea:** `rows-4 border #E2E8F0 radius-10 px-14 py-12 14px full-width`
placeholder "Chia sẻ trải nghiệm của bạn..." · counter `"0/500" 11px #94A3B8`

**Upload ảnh:** `border-2 dashed #E2E8F0 radius-10 p-16 text-center cursor-pointer`
- icon `add_photo_alternate 32px #94A3B8` + Text `"Thêm ảnh (tối đa 5)"`
- Preview grid: `grid-cols-5 gap-8` mỗi ảnh `60x60px radius-8` + nút `×`
- API: `POST /upload/images`

**Footer:** `flex justify-end gap-12 mt-16 pt-16 border-t #F1F5F9`
- "Hủy": `border #E2E8F0 bg white text #64748B`
- "Gửi đánh giá": `bg #0066CC text white` · disabled khi chưa chọn sao
  → `POST /ratings` body: `{ location_id, score, comment, images[], booking_id }`

### Đánh dấu đánh giá Hữu ích

Mỗi review có button:
- Chưa đánh dấu: `"Hữu ích (12)" 12px #94A3B8` icon `thumb_up_outlined` hover `#0066CC`
